...
float build_scaler = 12.0; // inches to pixels
float current_x, current_y, current_z;
float x_preview_offset = 15.0;
float y_preview_offset = 15.0;
float x_preview_offset_original, y_preview_offset_original, build_scaler_original;
float build_width = 20.0; // inches
float build_depth = 22.0; // inches
float x_max_ex = 0.0;
float x_max = 0.0;
float y_max_ex = 0.0;
float y_max = 0.0;
...
void keyPressed() { // anytime a key is pressed
...
void update_progress_bar(int current, int total) // progress bar for how much of file is sent
{
  int pb_height = 10;
  int pb_x = 125;
  int pb_y = 305;
  ...
  float progress_bar_width = (index*1.0/lines.length)*pb_total_width;
  fill(255);
...
int find_char(char[] command, char search_char, int j_start)
{
  int j = j_start;
...
float get_coordinate(String cmdstring, char search_char)
{
  ...
  if (find_char(command, search_char, 0) > 0) { // found the search character
    if (find_char(command, ' ', find_char(command, search_char, 0)) > 0) { // found where the space is (end of number)
      ...
      try {
        ...
        consolebox.scroll(distance);
        return -99999.0; // same as if we didn't find a coordinate
      }
    }
  }
  return -99997.0; // just use some random value to the program will know we didn't find a value
}



void gcode_interpreter(String cmdstring)
{
  ...
  if (cmdstring.indexOf("G20") != -1) { // found G20 for inch units
    ...
    if (preview == false) {
      ...
      current_temp = current_temp_int/1023.0*5.0/0.01;
  ...
  if (state != 7) { // not just an individual command
    ...
    if (cmdstring.indexOf("G0") != -1 || cmdstring.indexOf("G1") != -1){
      if (get_coordinate(cmdstring, 'X') != -99997.0) { // found an X value
        coordinates[1][preview_index] = get_coordinate(cmdstring, 'X');
        if (english_units == false) coordinates[1][preview_index] = coordinates[1][preview_index] / 25.4;
      ...
      if (get_coordinate(cmdstring, 'Y') != -99997.0) { // found a Y value
        coordinates[2][preview_index] = get_coordinate(cmdstring, 'Y');
        if (english_units == false) coordinates[2][preview_index] = coordinates[2][preview_index] / 25.4;
      ...
      if (get_coordinate(cmdstring, 'Z') != -99997.0) { // found a Z value
        coordinates[3][preview_index] = get_coordinate(cmdstring, 'Z');
        if (english_units == false) coordinates[3][preview_index] = coordinates[3][preview_index] / 25.4;
      ...
      if (extruding == true) { // are we extruding while making this move?
        coordinates[4][preview_index] = 1.0;
      }
      else {
        coordinates[4][preview_index] = 0.0;
      }
...
void open_gcode_preview_window()
{
...
class MyCanvas extends ControlWindowCanvas {
  public void draw(PApplet theApplet) {
    ...
    if ((theApplet.mousePressed == true) && (theApplet.mouseButton == RIGHT) && (Rmouse_pressed == false) && ((pw_height-theApplet.mouseY) > pw_lower_threshold)) { // first press
      ...
      build_scaler = build_scaler_original + (y_mouse_pos-theApplet.mouseY)/3.0;
